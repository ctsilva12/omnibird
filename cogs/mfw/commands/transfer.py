...
async def transfer(self, ctx, user: discord.Member | None = None, *values: str):
    ...
    try:
        mfws_to_transfer, inventory_map, guilds, emojis = await discordutils.parse_and_validate_mfws(self.bot, ctx.author, *values)
    ...
    async with db.transaction() as cur:
        await dbutils.get_user_info(user.id, cur=cur)
        ...
        await dbutils.cleanup_inventory(ctx.author.id)
